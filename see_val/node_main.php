...
$sql = "SELECT * FROM `val_water_see` WHERE date = '$date' ORDER BY `val_water_see`.`id_water` ASC";
  ...
  $sql = "SELECT * FROM `val_wather_see` WHERE `date_tmd` = '$date' ORDER BY `val_wather_see`.`date_tmd` ASC";
...
<h2 class="main-title">รายงานสถิติข้อมูล แม่น้ำชี อ.มหาชนะชัย</h2>
<!-- <h2 class="main-title">สถานีตรวจวัด อ่างเก็บน้ำห้วยวังนอง(NB01)</h2> -->
  ...
  <article class="white-block">
   ...
   <script>
       ...
       $.ajax({
           url: "see_val/data_history_grap_main.php?", //เรียกใช้งานไฟล์นี้
           data: "date="+m_y_date,
           type: "GET",
           async:false,
